...
const API_HOST = "https://events.datainpane.com";
// NOTE - this will use prod as the url for local reports
const KPIS_ENDPOINT = urljoin(
    environment.url ?? "https://cloud.datainpane.com/",
    "dip-kpis/",
);
//const KPIS_ENDPOINT = "http://localhost:8090/dip-kpis/";
const LOCK_NAME = "ph_datainpane_store";
...
/** Generic event handling via dip-server kpis endpoint, e.g. billing events */
const asyncDIPTrackEvent = async (
    event: string,
    properties: object,
...
export const trackLocalReportView = (
    ...
    if (!window.dipLocalViewEvent) {
        return;
    }
    asyncDIPTrackEvent(
        event,
        {
            author_id: window.dipAuthorId,
            report_id: window.dipReportId,
        },
        false,
    );
};

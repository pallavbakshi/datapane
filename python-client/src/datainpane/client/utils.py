...
log: logging.Logger = logging.getLogger("datainpane")
...
def _setup_dp_logging(verbosity: int = 0, logs_stream: t.TextIO | None = None) -> None:
    ...
    if get_dp_mode() == DPMode.LIBRARY:
        log.warning("Configuring datainpane logging in library mode")
        # return None

    # TODO - only allow setting once?
    if _have_setup_logging:
        log.warning(f"Reconfiguring datainpane logger when running as {get_dp_mode().name}")
        # raise AssertionError("Attempting to reconfigure datainpane logger")
        return None
    ...
    log_config = {
        ...
        "loggers": {"datainpane": {"level": log_level, "propagate": True}},
...
def print_debug_info():
    ...
    print("Data In Pane Debugging Info")
...
def open_in_browser(url: str):
    ...
    from datainpane.ipython.environment import get_environment

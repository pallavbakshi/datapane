"""
Data In Pane built-in helper functions to make creating your reports a bit simpler and reduce common tasks
"""
...
def build_md_view(
    ...
    **kwargs: b.BlockOrPrimitive,
    ...
    Returns:
        A Data In Pane App object for saving or uploading

    ..tip:: Either text or file is required as input
    ..tip:: Context, via args/kwargs can be plain Python objects, e.g. dataframes, and plots, or Data In Pane blocks, e.g. dip.Plot, etc.
...
def demo() -> Blocks:
    ...
    Returns:
        A Data In Pane View object for saving or uploading
    """
    ...
    def _gen_folium():
        return folium.Map(location=[45.372, -121.6972], zoom_start=12, tiles="OpenStreetMap")
    ...
    basics = """
This page describes Data In Pane, an API for creating data-driven reports from Python.
Data In Pane reports are comprised of blocks, which can be collected together and laid-out to form multiple-pages reports.
...
```python
dip.Plot(altair_plot, caption="A Plot")
```
...
Data In Pane has many other block types, including formulas, files, embeds, images, and big numbers - see the Blocks page for more info.
...
{{other}}
    ...
    """
    logo = ir.files("datainpane.resources") / "datainpane-icon-192x192.png"
    other = b.Group(
        b.Media(file=str(logo)),
        b.BigNumber(heading="Data In Pane Blocks", value=11, prev_value=6, is_upward_change=True),
    ...
    layout = """
Blocks on a page can be laid-out in Data In Pane using a flexible row and column system. furthermore, multiple blocks can be
nested into a single block where the user can select between which block, e.g. a plot, to view.
See https://docs.datainpane.com/reports/layout-and-customization for more info.
...
```python
dip.Group(plot1, plot2, columns=2)
cells = [dip.Text(f"### Cell {x}") for x in range(6)]
dip.Group(*cells, columns=0)  # 0 implies auto
```
...
```python
dip.Select(plot1, plot2, type=dip.SelectType.TABS)
dip.Select(plot1, plot2, type=dip.SelectType.DROPDOWN)
```
...
```python
group1 = dip.Group(plot1, plot2, columns=2)
dip.Select(group1, df)
```
...
"""
    ...
    adv_blocks = r"""
A list and demonstration of all the blocks supported by Data In Pane - see https://docs.datainpane.com/reports/blocks for more info.

## Plot Blocks

```python
dip.Group(dip.Plot(altair_plot, caption="Altair Plot"),
         dip.Plot(bokeh_plot, caption="Bokeh Plot"),
         dip.Plot(matplotlib_plot, caption="Matplotlib Plot"),
         dip.Plot(plotly_plot, caption="Plotly Plot"),
         dip.Plot(folium_plot, caption="Folium Plot"),
         columns=2)
```
...
```python
dip.Table(df, caption="Basic Table")
dip.Table(styled_df, caption="Styled Table")
dip.DataTable(df, caption="Interactive DataTable")
```
...
```python
dip.Text("Hello, __world__!")
dip.Code("print('Hello, world!')")
dip.Formula(r"\frac{1}{\sqrt{x^2 + 1}}")
dip.HTML("<h1>Hello World</h1>")
dip.BigNumber(heading="Data In Pane Blocks", value=11, prev_value=6, is_upward_change=True)
```
...
Files and Python objects can be added to a Data In Pane report, and be viewed (depending on browser support) and downloaded.

```python
dip.Media(file="./logo.png")
dip.Attachment(data=[1,2,3])
```
...
"""
    ...
    text = b.Group(
        ...
        b.BigNumber(heading="Data In Pane Blocks", value=11, prev_value=6, is_upward_change=True, name="big_num"),
        columns=0,
    )
    embed = b.Group(

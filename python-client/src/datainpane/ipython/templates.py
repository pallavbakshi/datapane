"""
Conversion templates for IPython notebooks to Data In Pane apps.
...
import datainpane.blocks as b
from datainpane.client.utils import display_msg
from datainpane.ipython.exceptions import BlocksNotFoundException
...
def partition_blocks_by_predicates(
    blocks: BaseElementList, predicates: list[BlockFilterF]
) -> list[BaseElementList]:
    """Partition blocks by predicates"""
    partitions: list[BaseElementList] = [[] for _ in range(len(predicates))]
...
def partition_blocks_by_types(
    ...
    partitions: list[BaseElementList] = partition_blocks_by_predicates(blocks, predicates)
...
class AssetListTemplate(IPythonTemplate, template_name="asset_list"):
    """Starts a new page for every supported Data In Pane block"""
...
class AssetCodeListTemplate(IPythonTemplate, template_name="asset_code_list"):
    """Starts a new page for every supported Data In Pane block, with the code block after the asset"""

    def transform(self) -> None:
        blocks = self.blocks
        pages: list[b.BaseBlock] = []
        last_block: b.BaseBlock | None = None

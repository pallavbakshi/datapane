...
def check_notebook_cache_parity(notebook_json: dict, ipython_input_cache: list) -> tuple[bool, list[int]]:
    ...
    for cell in notebook_json["cells"]:
        ...
        if (opt_out and "dp-exclude" not in tags) or (not opt_out and "dp-include" in tags):
            if (cell["cell_type"] == "markdown" and cell.get("source")) and (
                ...
                from datainpane.blocks.text import Text
                ...
                if "dp-show-code" in tags or show_code:
                    from datainpane.blocks.text import Code
